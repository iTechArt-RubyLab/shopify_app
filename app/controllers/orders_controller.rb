class OrdersController < ApplicationController
  ...
  def create
    ...
    if @shopify_order.save!
      ...
      @shopify_order.line_items.each do |line_item|
        params = line_item.except('id', 'admin_graphql_api_id', 'properties', 'tax_lines', 'price_set', 'duties',
                                  'discount_allocations', 'total_discount_set')
  ...
  def destroy
    @shopify_order = ShopifyAPI::Order.find(session: @session, id: @order.shopify_id)
    return unless @order.destroy && @shopify_order.delete
